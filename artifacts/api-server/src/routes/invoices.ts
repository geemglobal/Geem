...
async function buildInvoice(inv: typeof invoicesTable.$inferSelect) {
  ...
  if (inv.courierId) {
    const [courier] = await db.select({ id: couriersTable.id, name: couriersTable.name, apiProvider: couriersTable.apiProvider, trackingUrl: couriersTable.trackingUrl })
  ...
  return {
    ...
    courierProvider: inv.courierId ? (await db.select({ apiProvider: couriersTable.apiProvider }).from(couriersTable).where(eq(couriersTable.id, inv.courierId)))[0]?.apiProvider ?? null : null,
    courierName,
    courierTrackingUrl,
    items,
...
router.get("/invoices/by-number/:number/track", async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    courierProvider: inv.courierProvider,
