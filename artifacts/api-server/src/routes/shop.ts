...
router.get("/shop/orders/:orderNumber", async (req, res): Promise<void> => {
  ...
  if (wo.courierCn) {
    const [leopard] = await db
      .select({ id: couriersTable.id, name: couriersTable.name, apiProvider: couriersTable.apiProvider, trackingUrl: couriersTable.trackingUrl })
      .from(couriersTable)
      .where(eq(couriersTable.apiProvider, "leopard"));
    if (leopard) {
      courierName = leopard.name;
      courierProvider = leopard.apiProvider;
      courierId = leopard.id;
...
router.get("/shop/auth/orders", async (req, res): Promise<void> => {
  ...
  const result = await Promise.all(orders.map(async wo => {
    ...
    return {
      ...
      courierName: wo.courierCn ? "Leopard Courier" : null,
      courierProvider: wo.courierCn ? "leopard" : null,
...
router.get("/shop/auth/my-invoices", async (req, res): Promise<void> => {
  ...
  res.json(rows.map(r => {
    ...
    return {
      ...
      courierProvider: r.apiProvider ?? null,
