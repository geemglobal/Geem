...
function CurrencyDropdown({ selected, rate, loading, onSelect }: { selected: Currency; rate: number; loading: boolean; onSelect: (c: Currency) => void; }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(o => !o)} className="flex items-center gap-1.5 text-xs text-muted-foreground border rounded px-2 py-0.5 bg-gray-50 hover:bg-gray-100 transition-colors">
...
interface Customer { id: number; name: string; mobile: string; email?: string | null; address?: string | null; city?: string | null; }
interface LineItem {
  key: string; inventoryItemId: number | null; description: string; details: string; imei: string;
  deviceId: string | null; ptaStatus: string | null; qty: number; price: string; taxRate: number; taxLabel: string;
  searchQuery: string; searchOpen: boolean; isCreatingItem: boolean; newDesc: string; newPrice: string;
}

let _key = 0;
function newLine(): LineItem {
  return { key: String(++_key), inventoryItemId: null, description: "", details: "", imei: "", deviceId: null, ptaStatus: null, qty: 1, price: "", taxRate: 0, taxLabel: "", searchQuery: "", searchOpen: false, isCreatingItem: false, newDesc: "", newPrice: "" };
}

interface TaxOptionQ { id: string; label: string; detail: string; rate: number; }
const BUILT_IN_TAXES_Q: TaxOptionQ[] = [
  { id: "st18", label: "Sales Tax 18%", detail: "NTN: 6943433", rate: 18 },
];
let _sessionTaxesQ: TaxOptionQ[] = [];
function getAllTaxesQ() { return [...BUILT_IN_TAXES_Q, ..._sessionTaxesQ]; }
function addSessionTaxQ(t: TaxOptionQ) { _sessionTaxesQ = [..._sessionTaxesQ, t]; }

function TaxDropdown({ taxRate, taxLabel, onSelect }: {
  taxRate: number; taxLabel: string; onSelect: (rate: number, label: string) => void;
}) {
  const [open, setOpen] = useState(false);
  ...
  const [taxes, setTaxes] = useState<TaxOptionQ[]>(getAllTaxesQ);
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
  ...
  const filtered = taxes.filter(t =>
    t.label.toLowerCase().includes(search.toLowerCase()) || t.detail.toLowerCase().includes(search.toLowerCase()));
  function handleCreate() {
    const r = parseFloat(newRateStr);
    if (!newName.trim() || isNaN(r) || r <= 0) return;
    const t: TaxOptionQ = { id: `c_${Date.now()}`, label: newName.trim(), detail: "", rate: r };
    addSessionTaxQ(t); setTaxes(getAllTaxesQ()); onSelect(t.rate, t.label);
...
interface RowProps {
  line: LineItem; results: InvItem[]; isActive: boolean; onActivate: () => void;
  onUpdate: (p: Partial<LineItem>) => void; onRemove: () => void; onSelect: (item: InvItem) => void;
  onConfirmNew: () => void; canDelete: boolean; symbol: string; rate: number;
}
        ...
        <div className="absolute left-0 right-0 sm:left-5 sm:right-6 z-50 bg-white border rounded-lg shadow-xl overflow-hidden" style={{ top: "calc(100% - 4px)" }}>
          {!line.isCreatingItem ? (
                ...
                {results.map(item => (
                  <button key={item.id} className="w-full text-left px-4 py-2.5 hover:bg-blue-50/60 flex justify-between items-start gap-4 border-b last:border-0" onMouseDown={e => { e.preventDefault(); onSelect(item); }}>
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{item.deviceId ? `Device ID: ${item.deviceId}` : item.imei ? `${imeiLabel(item.imei)}: ${item.imei}` : item.brandName}</p>
                    </div>
            ...
            <div className="p-4 space-y-2.5">
              ...
              <input className="w-full px-3 py-1.5 text-sm border rounded outline-none focus:ring-1 focus:ring-blue-500" placeholder="Price *" value={line.newPrice} onChange={e => onUpdate({ newPrice: e.target.value.replace(/[^\d.]/g, "") })} onKeyDown={e => e.key === "Enter" && line.newDesc && line.newPrice && onConfirmNew()} />
              <div className="flex gap-2 pt-1">
...
export default function NewQuotation() {
  ...
  useEffect(() => {
    ...
    fetch("https://open.er-api.com/v6/latest/PKR")
      .then(r => r.json())
      .then((data: { rates: Record<string, number> }) => { if (data.rates) { Object.assign(ratesCache.current, data.rates); setExchangeRate(data.rates[selectedCurrency.code] ?? 1); } })
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    if (oldRate !== newRate && oldRate > 0 && newRate > 0) {
      setLines(ls => ls.map(l => { const old = parseFloat(l.price || "0"); if (old === 0) return l; const converted = (old / oldRate) * newRate; return { ...l, price: parseFloat(converted.toFixed(6)).toString() }; }));
    }
    prevRateRef.current = newRate;
  }, [exchangeRate]);

  const { data: custResults } = useQuery({
    queryKey: ["nq-cust", custQuery],
  ...
  const { data: editData, isLoading: editLoading } = useQuery({
    ...
    queryFn: () => axiosInstance.get(`/quotations/${editId}`).then(r => r.data),
    enabled: editMode,
  });

  useEffect(() => {
    if (!editData || initialized.current) return;
    initialized.current = true;
    setCustomer({ id: editData.customerId, name: editData.customerName, mobile: editData.customerPhone ?? "", email: null, address: editData.customerAddress ?? null, city: editData.customerCity ?? null });
    setQuotationNum(editData.quotationNumber);
    setQtDate(editData.date);
    setExpiryDate(editData.expiryDate ?? "");
    setNotes(editData.notes ?? "");
    setDiscount(editData.discount > 0 ? String(editData.discount) : "");
    if (editData.discount > 0) setDiscountOpen(true);
    const sh = parseFloat(String(editData.shipping ?? 0));
    setShipping(sh > 0 ? String(sh) : "");
    if (sh > 0) setShippingOpen(true);
    const cur = CURRENCIES.find(c => c.code === editData.currency) ?? CURRENCIES[0];
    setSelectedCurrency(cur);
    setLines(
      editData.items.length > 0
        ? editData.items.map((i: { description: string; imei: string | null; inventoryItemId: number | null; qty: number; price: number; taxRate: number; deviceId: string | null; ptaStatus: string | null }) => ({
            key: String(++_key), inventoryItemId: i.inventoryItemId, description: i.description, details: "",
            imei: i.imei ?? "", deviceId: i.deviceId, ptaStatus: i.ptaStatus,
            qty: i.qty, price: String(i.price), taxRate: i.taxRate,
            searchQuery: i.description, searchOpen: false, isCreatingItem: false, newDesc: "", newPrice: "",
          }))
        : [newLine()]
    );
  }, [editData]);
  ...
  const saveMutation = useMutation({
    mutationFn: (b: object) => axiosInstance.post("/quotations", b).then(r => r.data),
    onSuccess: (data) => { qc.invalidateQueries({ queryKey: ["quotations"] }); toast({ title: "Quotation saved!", description: data.quotationNumber }); navigate(`/quotations/${data.id}`); },
    onError: () => toast({ title: "Error saving quotation", variant: "destructive" }),
  });

  const updateMutation = useMutation({
    mutationFn: (b: object) => axiosInstance.put(`/quotations/${editId}`, b).then(r => r.data),
    onSuccess: (data) => { qc.invalidateQueries({ queryKey: ["quotations"] }); qc.invalidateQueries({ queryKey: ["quotation", String(editId)] }); toast({ title: "Quotation updated!", description: data.quotationNumber }); navigate(`/quotations/${data.id}`); },
  ...
  function selectItem(key: string, item: InvItem) {
    updateLine(key, {
      inventoryItemId: item.id, description: item.productName,
      details: [item.deviceId ? `Device ID: ${item.deviceId}` : "", item.imei ? `${imeiLabel(item.imei)}: ${item.imei}` : "", item.ptaStatus === "approved" ? "✓ PTA Approved" : ""].filter(Boolean).join("\n"),
      deviceId: item.deviceId ?? null, imei: item.imei ?? "", ptaStatus: item.ptaStatus ?? null,
      price: selectedCurrency.code === "PKR" || exchangeRate <= 0 ? String(item.sellingPrice) : parseFloat((item.sellingPrice * exchangeRate).toFixed(6)).toString(),
      searchQuery: item.productName, searchOpen: false, isCreatingItem: false,
    });
    setActiveKey(null);
  }
  ...
  for (const l of lines) {
    if (l.taxRate > 0 && l.taxLabel) {
      ...
      if (entry) { entry.amount += lineAmt * l.taxRate / 100; }
        ...
        const opt = getAllTaxesQ().find(t => t.rate === l.taxRate && t.label === l.taxLabel);
  ...
  function handleSave() {
    ...
    const payload = {
      customerId: customer.id, date: qtDate,
      expiryDate: expiryDate || null,
      quotationNumber: quotationNum || undefined,
      discount: String(discountAmt), shipping: String(shippingAmt), notes: notes || undefined,
      currency: selectedCurrency.code, currencySymbol: selectedCurrency.symbol,
      items: valid.map(l => ({ description: l.description, imei: l.imei || undefined, inventoryItemId: l.inventoryItemId || undefined, deviceId: l.deviceId || undefined, ptaStatus: l.ptaStatus || undefined, qty: l.qty, price: parseFloat(l.price), taxRate: l.taxRate })),
    };
    if (editMode) updateMutation.mutate(payload);
  ...
  return (
    <div className="max-w-3xl mx-auto pb-12 px-2 sm:px-0">
      <div className="flex items-center gap-2 mb-5">
        ...
        <span className="text-sm font-medium">{editMode ? `Edit ${quotationNum}` : "New Quotation"}</span>
      </div>
        ...
        <div className="grid grid-cols-1 gap-4 p-4 sm:grid-cols-5 sm:gap-6 sm:p-6 border-b">
            ...
            {customer ? (
              <div className="border rounded-lg p-4 min-h-[140px]">
                ...
                <div className="flex gap-3 mt-2.5 text-xs text-blue-600">
                  <button className="hover:underline" onClick={() => { setCustOpen(true); setCreatingCust(false); setCustQuery(""); }}>Choose a different customer</button>
                </div>
              </div>
            ) : (
              <div className="relative">
                <button onClick={() => { setCustOpen(true); setCreatingCust(false); }} className="w-full border-2 border-dashed rounded-lg min-h-[140px] flex flex-col items-center justify-center gap-2 text-blue-500 hover:border-blue-400 hover:bg-blue-50/40 transition-colors">
                          ...
                          <div className="relative">
                            ...
                            <input autoFocus className="w-full pl-8 pr-3 py-1.5 text-sm border rounded bg-white outline-none focus:ring-1 focus:ring-blue-500" placeholder="Search by name, mobile or address…" value={custQuery}
                              ...
                              onKeyDown={e => {
                                ...
                                else if (e.key === "Enter") { e.preventDefault(); const p = custList[custHighlight]; if (p) { setCustomer(p); setCustOpen(false); setCustQuery(""); } }
          ...
          <div className="sm:col-span-3 space-y-3">
            {[
              ...
              {
                label: "Expiry date", el: (
                  <div>
                    <input type="date" className="w-full px-3 py-1.5 text-sm border rounded outline-none focus:ring-1 focus:ring-blue-500" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} />
          ...
          <div>
            {lines.map(line => (
              <LineRow key={line.key} line={line} results={activeKey === line.key ? (itemResults?.items ?? []) : []} isActive={activeKey === line.key}
                onActivate={() => setActiveKey(line.key)} onUpdate={p => updateLine(line.key, p)}
                onRemove={() => setLines(ls => ls.filter(l => l.key !== line.key))} onSelect={item => selectItem(line.key, item)}
                onConfirmNew={() => confirmNewItem(line.key)} canDelete={true} symbol={selectedCurrency.symbol} rate={exchangeRate}
              />
            ))}
          </div>
          ...
          <div className="flex items-start">
            ...
            <div className="w-full sm:w-72 space-y-2 text-sm">
              ...
              {!discountOpen ? (
                ...
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">Discount{discountType === "percent" && parseFloat(discount || "0") > 0 ? ` (${parseFloat(discount || "0")}%)` : ""}</span>
                  <div className="flex items-center gap-1">
                    {discountType === "amount" && <span className="text-muted-foreground text-xs">{selectedCurrency.symbol}</span>}
                    <input autoFocus className="w-20 text-right px-2 py-0.5 text-sm border rounded outline-none focus:ring-1 focus:ring-blue-500" placeholder="0.00" value={discount} onChange={e => setDiscount(e.target.value.replace(/[^\d.]/g, ""))} />
                    ...
                    <button onClick={() => { setDiscountOpen(false); setDiscount(""); setDiscountType("amount"); }}><X className="h-3.5 w-3.5 text-muted-foreground hover:text-destructive" /></button>
                  </div>
                </div>
              )}
              {taxBreakdown.map(t => (
              ...
              <div className="flex justify-between items-center font-bold text-base border-t pt-2">
                ...
                <div className="text-right">
                  ...
                  {selectedCurrency.code !== "PKR" && exchangeRate > 0 && (
                    <div className="text-xs font-normal text-muted-foreground mt-0.5">≈ Rs{fmt(total / exchangeRate)} PKR</div>
                  )}
                </div>
              </div>
            </div>
