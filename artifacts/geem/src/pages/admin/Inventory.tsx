                     ...
                     {liveTracking.data && (
                       <>
                         ...
                         {liveTracking.data.details.length > 0 && (
                          ...
                          {(liveTracking.data.origin || liveTracking.data.destination || liveTracking.data.shipper || liveTracking.data.consignee) && (
                            <div className="grid grid-cols-2 gap-2 rounded-md border border-blue-100 bg-white p-2.5 text-xs">
                              {[
                                ["From", liveTracking.data.origin],
                                ["To", liveTracking.data.destination],
                                ["Shipper", liveTracking.data.shipper],
                                ["Receiver", liveTracking.data.consignee],
                                ["Pieces", liveTracking.data.pieces],
                                ["Weight", liveTracking.data.packetWeight],
                          ...
                          {(liveTracking.data.signedFor || liveTracking.data.deliveredAt || liveTracking.data.deliveredTo) && (
                            <div className="rounded-md border border-green-200 bg-green-50 p-2.5 text-xs">
                              <p className="font-semibold text-green-900 mb-2">Delivery confirmation</p>
                              <div className="grid grid-cols-2 gap-2">
                                {[
                                  ["Signed for by", liveTracking.data.signedFor],
                                  ["Delivered date & time", liveTracking.data.deliveredAt],
                                  ["Delivered to", liveTracking.data.deliveredTo],
                                  ["Reference", liveTracking.data.referenceNo],
                                ].filter((entry): entry is [string, string] => Boolean(entry[1])).map(([label, value]) => (
                                  <div key={label}>
                                    <p className="text-green-800/70">{label}</p>
                                    <p className="font-medium text-green-950">{value}</p>
                                  </div>
                                ))}
                              </div>
                            </div>
                          )}
                          {(liveTracking.data.events?.length ?? 0) > 0 && (
                            <div className="rounded-md border border-blue-100 bg-white divide-y text-xs">
                              <p className="px-2.5 py-2 font-semibold text-blue-900">Shipment activity</p>
                              {liveTracking.data.events?.map((event, index) => (
                                <div key={`${event.date}-${event.time}-${index}`} className="px-2.5 py-2">
                                  <p className="font-medium">{event.description}</p>
                                  <p className="text-muted-foreground">{event.date}{event.time ? ` · ${event.time}` : ""}</p>
                                </div>
                              ))}
                            </div>
                          )}
